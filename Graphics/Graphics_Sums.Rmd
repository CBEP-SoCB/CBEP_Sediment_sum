...
them in TEQ -- toxic equivalents.  Toxic equivalents provide a way to estimate
the cumulative toxic effect of a mixture of related chemicals by weighting each
...
obvious whether we should prepare panels by region or by parameter, so we try
both.

```{r three_organics_graphic, fig.width = 7, fig.height = 5}
sums_data %>%
  filter(Parameter %in%
...
```{r four_organics_graphics, fig.width = 7, fig.height = 4}
sums_data %>%
  filter(Parameter %in%
           c("Total PAHs", "Total PCBs", "DDT Residues",
             "Dioxins and Furans")) %>%
...
```{r five_regions, fig.width = 7, fig.height = 4}
sums_data %>%
...
```{r five_regions_by_parm, fig.width = 7, fig.height = 3.5}
sums_data %>%
  ggplot(aes(x = Region, y = Result)) +
  geom_jitter(aes(color = LVL), size = 2, alpha = 0.35, 
              width = 0.2, height = 0) +
...
We don't have sufficient detections to show a similar graphic for just the
2010s. The Ramboll study treated non-detects as missing, thus biasing estimation
of sums, and failing to provide values to graph for many observations from
recent years, when non-detects were common.  We have chosen not to recalculate
totals, to minimize changes from the Ramboll report, but these graphics show why
that may be problematic.
```{r five_region_by_parm_recent, fig.width = 7, fig.height = 3.5}
sums_data %>%
...
# Trend Graphics
```{r trend_plot_base, fig.width = 7, fig.height = 3.5}
plt <- sums_data %>%
...
```{r trend_graphic_final, fig.width = 7, fig.height = 3.5}
